...
// Non-release publication identifiers, such as commit SHAs, may supply a
// separate numeric Apple version without changing their Maven coordinates.
private val frameworkShortVersion = providers.gradleProperty("APPLE_FRAMEWORK_VERSION")
    .orElse(project.version.toString().substringBefore('-'))
    .get()
...
require(frameworkShortVersion.matches(Regex("[0-9]+(?:\\.[0-9]+){0,2}"))) {
    "APPLE_FRAMEWORK_VERSION or VERSION_NAME must provide a numeric Apple framework version"
}
