...
4. After the first successful hosted CI run, protect `main` and require these
   exact checks before changes can merge:

   - `Kotlin, Android host, and JVM coverage`
   - `Android runtime (API 21)`
   - `Android runtime (API 36)`
   - `Kotlin/Native and Swift`

   Land later release changes, including the candidate checksum, through a
   pull request so the protected checks evaluate the exact merge candidate.
...
9. After the first green `main` build, open the public repository on JitPack,
   request the exact commit SHA, and inspect its log. Before building the first
   release candidate, use clean dependency caches to resolve the direct
   `com.github.yannallain.rrule-kmp:rrule-kmp:<sha>` module, verify its
   `.module` component group, run the standalone JVM test, compile all three
   Apple KLIB variants, and build the full-R8 Android consumer. A successful
   JitPack build log alone does not prove Gradle variant resolution. After
   publication, request the numeric version as well; the distribution-smoke
   workflow remains the authoritative public-consumer gate. JitPack needs no
...
Commit the manifest and documentation on a release-preparation branch, open a
pull request, and merge it only after every required CI job passes. Do not
create or push the tag manually. After merging, wait for the CI `push` run on
the resulting `main` commit to pass; the release workflow requires that exact
hosted result. Run the owner-provided real iOS 13 runtime/device check now, or
keep the documented caveat if that environment is still unavailable.
...
```kotlin
implementation(
    "com.github.yannallain.rrule-kmp:rrule-kmp:<version>"
)
```
